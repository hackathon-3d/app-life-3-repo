...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithHexString:@"DADADA" withAlpha:1.0].CGColor);
...
//    }
    UIImage *dragHandle = [UIImage imageNamed:@"dragHandle.png"];

        CGRect allPoints[8] = { upperLeft, upperRight, lowerRight, lowerLeft, upperMiddle, lowerMiddle, middleLeft, middleRight };
    for (NSInteger i = 0; i < 8; i++) {
            CGRect currPoint = allPoints[i];
        [dragHandle drawInRect:currPoint];
    }
...
- (void)translateUsingTouchLocation:(CGPoint)touchPoint {
    ...
    if (self.preventsPositionOutsideSuperview) {
        ...
        if ([self.superview isKindOfClass:[UIScrollView class]]) {
            ...
            bounds = CGRectMake(0, 0, superScroll.contentSize.width, superScroll.contentSize.height);
        }
        
        CGFloat midPointX = CGRectGetMidX(self.bounds);
        if (newCenter.x > bounds.size.width - midPointX) {
            newCenter.x = bounds.size.width - midPointX;
        }
        if (newCenter.x < midPointX) {
            newCenter.x = midPointX;
        }
        CGFloat midPointY = CGRectGetMidY(self.bounds);
        if (newCenter.y > bounds.size.height - midPointY) {
            newCenter.y = bounds.size.height - midPointY;
        }
